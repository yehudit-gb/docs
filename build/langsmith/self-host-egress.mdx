...
* **Operational LangSmith log messages**: warning and error log messages only. These help our support and engineering teams diagnose and troubleshoot issues more effectively.
...
You can disable all telemetry by setting the following values in your `langsmith_config.yaml` file (you may also disable individual telemetry types):
...
### Telemetry: Operational LangSmith Log Messages

We only export warning and error log messages from self-hosted LangSmith instances. This allows the LangChain team to better troubleshoot application errors without requiring back-and-forth communication with your team.

**Endpoint:**

`POST beacon.langchain.com/v1/beacon/v1/logs`
...
LangChain will not store any sensitive information in the Subscription Metrics, Operational Metadata, or Log Messages. Any data collected will not be shared with a third party. Log messages are filtered to only include warning and error severity levels, and we do not capture log messages that could contain sensitive application data. If you have any concerns about the data being sent, disable telemetry and/or reach out to your account team.
