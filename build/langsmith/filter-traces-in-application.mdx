...
To filter either inputs or outputs, you can use the **<Icon icon="magnifying-glass"/> Full-Text Search** filter, which will match keywords in either field. For a more targeted search, you can use the **{<Icon icon="arrow-down-right"/>} Input** or **<Icon icon="arrow-up-left"/> Output** filters, which will only match content based on the respective field.
...
If you have [copied a previously constructed filter](#copy-the-filter), you may want to manually apply this raw query in a future session.
