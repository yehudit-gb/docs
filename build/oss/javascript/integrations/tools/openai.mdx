...
sidebarTitle: "Tools"
description: "Integrate with the OpenAI tool using LangChain JavaScript."
---

The `@langchain/openai` package provides LangChain-compatible wrappers for OpenAI's built-in tools. These tools can be bound to `ChatOpenAI` using `bindTools()` or [`createAgent`](https://reference.langchain.com/javascript/functions/langchain.index.createAgent.html).
