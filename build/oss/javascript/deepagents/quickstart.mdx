...
1. **Plans its approach** using the built-in [`write_todos`](/oss/javascript/deepagents/harness#to-do-list-tracking) tool to break down the research task.
1. **Conducts research** by calling the `internet_search` tool to gather information.
1. **Manages context** by using file system tools ([`write_file`](/oss/javascript/deepagents/harness#file-system-access), [`read_file`](/oss/javascript/deepagents/harness#file-system-access)) to offload large search results.
